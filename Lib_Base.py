...
class Lib_Base(FwTestLib):
    ...
    def __init__(self, xlsReaderObj: ConfigExlParser, reportUpdaterObj: ReportExlParser, fbTestModeList: [],
        ...
        self.__initiateSymbolicVariables()
        self.connect()
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def __get_session_numbers(self) -> range:
        """
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def download_PLCD_Projects(self) -> None:
        """
        ...
        for index, ipcObj in enumerate(self.ipcObjs):
            ...
            if index == 0:
                ...
                self.logger.newStep(f"Download Boot Project in host controller {currControllerType} with IP Address "
                                    f"{currIpAddr} from path {relBootProjectPath}")
            else:
                relBootProjectPath = "..\\projects\\plcd\\server\\" + f"{currControllerType}" + "\\boot"
                self.logger.newStep(f"Download Boot Project in client controller {currControllerType} with IP Address "
                                    f"{currIpAddr} from path {relBootProjectPath}")
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def _execute_Grp(self, testType: str) -> Tuple[List[bool], List[List[str]]]:
        """
        ...
        for fbCurrentMode in self.fbTestModeList:
            ...
            result = self.verify_OpenPLC_Client_Function_Block(testType = testType)
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def _execute_AuthGrp(self, testType: str, authType: FBTokenType) -> Tuple[List[bool], List[List[str]]]:
        """
        Execute a group of Function Block (FB) tests for a given test type.
        ...
        for fbCurrentMode in self.fbTestModeList:
            ...
            # Enable the sessions
            self.executeSessions(executeValue = True)
            result = self.verify_OpenPLC_Client_AuthToken_Function_Block(testType = testType)
    ...
    def mapSessionIPAddresses(self, testType: str) -> None:
        ...
        for sessionNumber in self.__get_session_numbers():
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def executeSessions(self, executeValue: bool):
        """
        This method is used to set FB client urls for Open PLC connections
        """
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def __validateStepNumbersUpdating(self) -> None:
        """
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def __validateCycleCountUpdating(self) -> None:
        """
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def __validateConnectStatus(self) -> None:
        """
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def __verifyFailedStepAfterConnect(self) -> bool:
        """
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def __checkFailedStepErrorId(self) -> []:
        """
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def __verifyReadVariables(self) -> bool:
        """
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def __verifyWriteVariables(self, isAuthTestRemarks: bool = False) -> bool:
        """
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def __verifyReadWriteVariables(self, isAuthTestRemarks: bool = False) -> bool:
        """
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def __verifyMethodExecution(self) -> bool:
        """
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def verify_OpenPLC_Client_Function_Block(self, testType: str) -> []:
        """
        ...
        while waitIndex <= waitLoop:
            ...
            returnResult.append(self.__verifyReadWriteVariables())

            if self.fbTestImplementation == constants.FBImplementation.VARIABLE_RW_METHOD_CONNECT:
                returnResult.append(self.__verifyMethodExecution())
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def verify_OpenPLC_Client_AuthToken_Function_Block(self, testType: str) -> []:
        """
        This method verify the Function blocks for the operation types Read, Write,
        Asynchronous Write & Read and Method call
        ...
        while waitIndex <= waitLoop:
            ...
            if not self.__verifyFailedStepAfterConnect():
            ...
            self.logger.info("---------- [Verification]: Step3 - Check for error id while reading the values "
                             "from variables for every connected session ----------")
            testResult = self.__verifyReadVariables()

            self.logger.info("---------- [Verification]: Step4 - Check for error id while writing the values to "
                             "variables for every connected session ----------")
            testResult = testResult and self.__verifyWriteVariables(isAuthTestRemarks = True)

            self.logger.info("---------- [Verification]: Step5 - Check for error id while writing & reading the "
                             "values to & from variables for every connected session ----------")
            testResult = testResult and self.__verifyReadWriteVariables(isAuthTestRemarks = True)
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def __initiateSymbolicVariables(self):
        """
        Initialize per-session PLC Open Client/Server symbolic variables by resolving memory objects.

        This method iterates over all session numbers returned by `self.__get_session_numbers()`
        ...
        for sessionNo in self.__get_session_numbers():
